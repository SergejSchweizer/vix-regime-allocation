...
def _dual_comparison() -> pd.DataFrame:
    index = pd.date_range("2026-02-02", periods=4, freq="B", name="Date")
    return pd.DataFrame(
        {
        ...
        index=index,
        columns=list(module.DUAL_COMPARISON_COLUMNS),
    )


def test_four_portfolio_summary_has_exact_schema_and_order() -> None:
    summary = module.build_four_portfolio_performance_summary(_dual_comparison())
    assert tuple(summary.columns) == SUMMARY_COLUMNS
    assert summary["portfolio"].tolist() == list(module.DUAL_COMPARISON_COLUMNS)
    assert summary["observations"].tolist() == [4, 4, 4, 4]


def test_four_portfolio_summary_delegates_shared_metric_function(
    ...
    def fake_metrics(series: pd.Series) -> dict[str, float | int]:
        ...
        return {
            ...
            "annualized_return": value + 1.0,
            "annualized_volatility": value + 2.0,
            "sharpe_ratio": value + 3.0,
            "max_drawdown": -value,
            "observations": len(series),
        }
    ...
    monkeypatch.setattr(module, "performance_metrics", fake_metrics)
    summary = module.build_four_portfolio_performance_summary(_dual_comparison())
    assert seen == list(module.DUAL_COMPARISON_COLUMNS)
    assert summary["portfolio"].tolist() == list(module.DUAL_COMPARISON_COLUMNS)
...
def test_strict_four_portfolio_api_rejects_bad_schema(mutator) -> None:  # type: ignore[no-untyped-def]
    with pytest.raises((TypeError, ValueError)):
        module.build_four_portfolio_performance_summary(mutator(_dual_comparison()))


def test_dispatcher_uses_four_portfolio_path_for_dual_comparison() -> None:
    direct = module.build_four_portfolio_performance_summary(_dual_comparison())
    dispatched = build_performance_summary(_dual_comparison())
