...
def test_spy_buy_hold_is_exact_spy_simple_return() -> None:
    data = _data()
    result = build_spy_buy_hold_returns(data, data.index)
...
@pytest.mark.parametrize("case", ["missing", "duplicate", "unsorted", "wrong_name"])
...
def test_benchmarks_reject_noncanonical_data() -> None:
