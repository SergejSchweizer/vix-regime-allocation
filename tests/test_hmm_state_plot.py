...
def test_plot_writes_nonempty_file_and_closes_figure(tmp_path: Path) -> None:
    ...
    output = tmp_path / "nested" / "hmm_states.png"
    before = set(plt.get_fignums())
    plot_hmm_vix_states(vix, states_2, states_3, output)
    assert output.exists() and output.stat().st_size > 0
    assert set(plt.get_fignums()) == before


def test_plot_contains_vix_and_each_state_once(
...
def test_index_and_data_validation_failures(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="exactly match"):
        plot_hmm_vix_states(vix, shifted, states_3, tmp_path / "x.png")

    wrong_name = states_2.rename("wrong")
    with pytest.raises(ValueError, match="named"):
        plot_hmm_vix_states(vix, wrong_name, states_3, tmp_path / "x.png")

    bad_labels = states_3.copy()
    bad_labels.iloc[0] = 3
    with pytest.raises(ValueError, match="0..2"):
        plot_hmm_vix_states(vix, states_2, bad_labels, tmp_path / "x.png")

    bad_dtype = states_2.astype(float)
    with pytest.raises(ValueError, match="integer"):
        plot_hmm_vix_states(vix, bad_dtype, states_3, tmp_path / "x.png")

    nonfinite = vix.copy()
    nonfinite.iloc[0] = np.nan
    with pytest.raises(ValueError, match="finite"):
        plot_hmm_vix_states(nonfinite, states_2, states_3, tmp_path / "x.png")
