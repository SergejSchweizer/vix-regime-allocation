...
def data_source_and_financial_interpretation_002() -> Any:  # pragma: no cover
    ...
    global Image, Markdown, OUTPUT_COLUMNS, Path, TemporaryDirectory, data, data_path, display, np, pd, plot_etf_log_returns, plot_vix_change, re, references_path, repo_root
    import re
...
def data_source_and_financial_interpretation_003() -> Any:  # pragma: no cover
    ...
    sample_summary = pd.DataFrame({'value': [data.index.min().date().isoformat(), data.index.max().date().isoformat(), len(data), int(data.isna().sum().sum())]}, index=['start_date', 'end_date', 'observations', 'missing_values'])
...
def daily_change_in_vix_007() -> Any:  # pragma: no cover
    ...
    return display(Markdown(f'**Computed Step 1 observations.** The common sample contains **{len(data)}** rows from **{data.index.min().date().isoformat()}** through **{data.index.max().date().isoformat()}**. The largest absolute daily VIX change occurs on **{vix_extreme_date.date().isoformat()}** and equals **{repr(vix_extreme_change)}** VIX points. Among the three ETF log-return series, **{highest_std_series}** has the largest sample daily standard deviation, **{repr(highest_std_value)}**, using `ddof=1`.'))
...
def step_2_modeling_vix_regimes_discrete_markov_chai_014() -> Any:  # pragma: no cover
    ...
    global candidate, diagnostics, evaluate_markov_candidate, k, markov_2, markov_3, markov_candidates, markov_figure_dir, markov_figure_path, markov_output_dir, plot_markov_vix_states
    ...
    for k, candidate in markov_candidates.items():
        candidate['thresholds'].to_csv(markov_output_dir / f'step2_markov_{k}_thresholds.csv', index=False)
        candidate['transition'].reset_index().to_csv(markov_output_dir / f'step2_markov_{k}_transition.csv', index=False)
        candidate['stationary'].reset_index().to_csv(markov_output_dir / f'step2_markov_{k}_stationary.csv', index=False)
        candidate['states'].rename_axis('Date').reset_index().to_csv(markov_output_dir / f'step2_markov_{k}_states.csv', index=False)
    ...
    for k, candidate in markov_candidates.items():
        ...
        diagnostics = pd.Series({'conditional_log_likelihood': candidate['log_likelihood'], 'transition_observations': candidate['n_observations'], 'free_transition_parameters': candidate['n_parameters']}, name='value')
...
def step_2_modeling_vix_regimes_gaussian_hidden_mark_017() -> Any:  # pragma: no cover
    ...
    global _hmm_parameter_table, candidate, diagnostics, evaluate_hmm_candidate, fit, hmm_2, hmm_3, hmm_candidates, hmm_figure_dir, hmm_output_dir, hmm_probability_figure_path, hmm_state_figure_path, k, parameters, plot_hmm_smoothed_probabilities, plot_hmm_vix_states
    ...
    def _hmm_parameter_table(fit):
        ...
        return pd.DataFrame({'state': range(fit.n_states), 'mean_vix_change': fit.means.to_numpy(dtype=float), 'variance_vix_change': fit.variances.to_numpy(dtype=float), 'start_probability': list(fit.start_probabilities), 'viterbi_observations': counts.to_numpy(dtype=int), 'viterbi_occupancy': counts.to_numpy(dtype=float) / len(fit.states), 'posterior_mean_probability': [float(posterior_means[f'state_{state}']) for state in range(fit.n_states)]})
    for k, candidate in hmm_candidates.items():
        ...
        fit.transition_matrix.reset_index().to_csv(hmm_output_dir / f'step2_hmm_{k}_transition.csv', index=False)
        fit.states.rename_axis('Date').reset_index().to_csv(hmm_output_dir / f'step2_hmm_{k}_states.csv', index=False)
        ...
        diagnostics = pd.Series({'selected_restart_seed': fit.seed, 'converged': fit.converged, 'log_likelihood': candidate['log_likelihood'], 'observations': candidate['n_observations'], 'free_parameters': candidate['n_parameters']}, name='value')
        display(diagnostics.to_frame())
    ...
    plot_hmm_vix_states(data['VIX'], hmm_2['fit'].states, hmm_3['fit'].states, hmm_state_figure_path)
    plot_hmm_smoothed_probabilities(hmm_2['fit'].probabilities, hmm_3['fit'].probabilities, hmm_probability_figure_path)
...
def step_3_model_selection_and_selected_state_proven_020() -> Any:  # pragma: no cover
    ...
    global COMPARISON_COLUMNS, build_model_comparison, comparison_path, hmm_candidate_list, input_sha256, json, k, markov_candidate_list, select_preferred_model, selected_model, selected_model_path, selected_states, selected_states_path, sha256, source_states, state_source_path, step3_comparison, step3_generated_dir, step3_selection, step3_table_dir
    import json
    from hashlib import sha256
    from vix_regime_allocation.model_selection import COMPARISON_COLUMNS, build_model_comparison, select_preferred_model
    ...
    step3_selection = select_preferred_model(step3_comparison, markov_candidate_list, hmm_candidate_list)
    ...
    selected_model = {'family': step3_selection['family'], 'n_states': int(step3_selection['n_states']), 'state_source': step3_selection['state_source'], 'selection_reason': step3_selection['selection_reason'], 'markov_best_n_states': int(step3_selection['markov_best_n_states']), 'hmm_best_n_states': int(step3_selection['hmm_best_n_states']), 'input_data_sha256': input_sha256, 'selected_states_path': 'reports/tables/step3_selected_states.csv'}
    assert tuple(selected_model) == ('family', 'n_states', 'state_source', 'selection_reason', 'markov_best_n_states', 'hmm_best_n_states', 'input_data_sha256', 'selected_states_path')
    ...
    return display(Markdown(f"**Selected model:** {selected_model['family'].upper()} with $K={selected_model['n_states']}$. {selected_model['selection_reason']}"))
...
def step_3_state_conditional_etf_analysis_023() -> Any:  # pragma: no cover
    ...
    global compute_state_asset_statistics, expected_rows, plot_state_asset_statistics, selected_state_frame, selected_states_for_statistics, selected_states_path, state_asset_statistics, state_leaders, statistics_figure_path, statistics_path
    ...
    state_leaders = state_asset_statistics.sort_values(['state', 'mean_log_return', 'asset'], ascending=[True, False, True], kind='stable').groupby('state', sort=True, as_index=False).first()[['state', 'asset', 'mean_log_return', 'std_log_return', 'observations']]
...
def step_4_state_based_rotation_rule_026() -> Any:  # pragma: no cover
    ...
    global ALLOCATION_COLUMNS, build_state_allocation, row, step3_statistics, step3_statistics_path, step4_allocation, step4_allocation_path
    ...
    assert np.allclose(step4_allocation[['TLT_weight', 'GLD_weight', 'SPY_weight']].sum(axis=1), 1.0)
...
def deterministic_steps_2_4_artifact_manifest_029() -> Any:  # pragma: no cover
    ...
    global figures, hashlib, input_sha256, json, manifest, manifest_path, relative_path, selected_model, selected_model_path, step1_path, tables
    import hashlib
    import json
    ...
    tables = sorted(['reports/tables/step2_hmm_2_parameters.csv', 'reports/tables/step2_hmm_2_states.csv', 'reports/tables/step2_hmm_2_transition.csv', 'reports/tables/step2_hmm_3_parameters.csv', 'reports/tables/step2_hmm_3_states.csv', 'reports/tables/step2_hmm_3_transition.csv', 'reports/tables/step2_markov_2_states.csv', 'reports/tables/step2_markov_2_stationary.csv', 'reports/tables/step2_markov_2_thresholds.csv', 'reports/tables/step2_markov_2_transition.csv', 'reports/tables/step2_markov_3_states.csv', 'reports/tables/step2_markov_3_stationary.csv', 'reports/tables/step2_markov_3_thresholds.csv', 'reports/tables/step2_markov_3_transition.csv', 'reports/tables/step3_model_comparison.csv', 'reports/tables/step3_selected_states.csv', 'reports/tables/step3_state_asset_statistics.csv', 'reports/tables/step4_allocation_mapping.csv'])
    figures = sorted(['reports/figures/step2_hmm_smoothed_probabilities.png', 'reports/figures/step2_hmm_vix_states.png', 'reports/figures/step2_markov_vix_states.png', 'reports/figures/step3_state_asset_statistics.png'])
    for relative_path in tables + figures:
        assert (repo_root / relative_path).is_file(), relative_path
    manifest = {'schema_version': 1, 'input_data_path': 'data/processed/step1_data.csv', 'input_data_sha256': input_sha256, 'notebook_path': 'notebooks/gwp2_vix_regime_allocation.ipynb', 'selected_model_path': 'reports/generated/step3_selected_model.json', 'tables': tables, 'figures': figures}
...
def step_5_backtest_construction_and_required_benchm_031() -> Any:  # pragma: no cover
    ...
    global COMPARISON_COLUMNS, Path, ROTATION_DETAIL_COLUMNS, allocation, allocation_path, build_comparison, build_equal_weight_monthly_returns, build_rotation_returns, build_spy_buy_hold_returns, comparison_index, daily_returns_path, equal_weight, hashlib, input_sha256, json, repo_root, rotation_detail, selected_model, selected_model_path, selected_states, selected_states_frame, selected_states_path, spy_buy_hold, step1, step1_path, step5_daily_returns
    import hashlib
    import json
    from pathlib import Path
    ...
    from vix_regime_allocation.benchmarks import build_equal_weight_monthly_returns, build_spy_buy_hold_returns
    ...
    selected_states = pd.Series(selected_states_frame['state'].to_numpy(dtype=int), index=step1.index, name='state', dtype=int)
    ...
    return display(Markdown(f'Comparison period: **{comparison_index[0].date()}** to **{comparison_index[-1].date()}**, observations: **{len(comparison_index)}**.'))
...
def step_5_performance_metrics_and_cumulative_compar_034() -> Any:  # pragma: no cover
    ...
    global build_performance_summary, daily_path, figure_path, label, lines, performance_summary, plot_cumulative_performance, portfolio, repo_root_step5, row, rows, step5_daily, summary_path
    ...
    lines = []
    for portfolio, label in [('regime_rotation', 'Regime rotation'), ('equal_weight_monthly', 'Equal-weight monthly reset'), ('spy_buy_hold', 'SPY buy and hold')]:
        row = rows.loc[portfolio]
        lines.append(f"- **{label}:** cumulative return {row['cumulative_return']:.6f}; annualized return {row['annualized_return']:.6f}; annualized volatility {row['annualized_volatility']:.6f}; Sharpe {row['sharpe_ratio']:.6f}; maximum drawdown {row['max_drawdown']:.6f}; observations {int(row['observations'])}.")
    return display(Markdown('### Numerical comparison\n' + '\n'.join(lines)))

def works_cited_037() -> Any:  # pragma: no cover
    """Notebook section: Works Cited (cell 37)."""
    global _mla_author, _mla_entry, _parse_bibtex_registry, cited_keys, key, missing_keys, references, references_path
    references_path = repo_root / 'reports/references.bib'
    ...
    cited_keys = ['whaley2009vix', 'cboe2019vixfaq', 'baum1970maximization', 'rabiner1989tutorial', 'viterbi1967decoding', 'akaike1974identification', 'schwarz1978dimension', 'markowitz1952portfolio', 'white2000datasnooping', 'bailey2014deflatedsharpe']
    ...
    return display(Markdown(chr(10).join((f'- {_mla_entry(references[key])}' for key in cited_keys))))
